...
const FavouriteSlice = createSlice({
  ...
  reducers: {
    ...
    setshow: (state,action) =>{
      state.singleFavUser = state.favouriteUsers.filter((user)=>user.id === action.payload)
...
export const { addFavourite, removeFavourite,showFavouriteUsers,editFavourite,setStatus,setshow } = FavouriteSlice.actions;
  ...
  return async function fetchFavouriteUsersThunk(dispatch) {
    ...
    try {
      ...
      const response = await fetch(
        `https://67f0b7fe2a80b06b8898713e.mockapi.io/contacts/favourites`
      );
...
export function deleteFavouriteUser(id) {
  ...
  return async function deleteUserThunk(dispatch) {
    ...
    try {
      await fetch(
        `https://67f0b7fe2a80b06b8898713e.mockapi.io/contacts/favourites/${id}`,
        {
          method: "DELETE",
        }
      );
